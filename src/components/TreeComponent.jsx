...
const TreeComponent = (props) => {
    ...
    return new Promise((resolve) => {
      ...
      if (treeNode.dataRef.parent_id) {
        API.get(
          ...
          const resData = res.data.data.supersub_categories_List.categories.map(
            (category) => ({
              title: category.category_name,
              key: category.id,
              parent_id: category.parent_id,
  ...
  return (
    <Tree loadData={handleLoadData} onSelect={clickHandler} defaultExpandAll className="custom-tree"  titleRender={(node) => (
